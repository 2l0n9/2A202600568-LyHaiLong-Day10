...
Số lượng record theo doc_id trong file CSV thô:

hr_leave_policy: 40
policy_refund_v4: 33
sla_p1_2026: 31
legacy_catalog_xyz_zzz: 31
data_privacy_guideline: 29
it_helpdesk_faq: 26
security_policy: 18
access_control_sop: 8
Ngoài ra còn có rất nhiều mã rác kiểu invalid_doc_*.
...
có 2 dòng trong hr_leave_policy chứa dữ liệu cũ ("10 ngày phép năm") lọt qua được bộ lọc.
- **So sánh** `doc_id` trong CSV vs `ALLOWED_DOC_IDS` trong `cleaning_rules.py` → phát hiện nguồn bị thiếu.
transform/cleaning_rules.py, ALLOWED_DOC_IDS hiện tại chỉ cho phép 4 nguồn đầu tiên->thiếu mất nguồn access_control_sop.
- **Đối chiếu** `expect_top1_doc_id` trong `grading_questions.json` → xác nhận cần sửa gì.
Thêm access_control_sop vào danh sách ALLOWED_DOC_IDS.
Thêm cleaning rule để xử lý dữ liệu lỗi trong hr_leave_policy (chứa text "10 ngày phép năm") để pipeline không còn bị HALT nữa
